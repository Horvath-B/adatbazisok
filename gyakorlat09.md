...
with p.name as Nev, count(m.title) as Darab //segéd lekérdezés, having helyett
where Darab > 1
return Nev, Darab
```
...
```js
match (p:Person)-[a:ACTED_IN]->(m:Movie)<-[:DIRECTED]-(p:Person)
return p.name
```
```js
...
```
```js
// legtöbb lehetőséget ez biztosítja, tagadni is lehet
match (p:Person)-[a:ACTED_IN]->(m:Movie)
where exist ((p:Person)-[:DIRECTED]->(m:Movie))
return p.name
```
...
```js
profile //végrehajtási tervet készít
match (p:Person)
where p.born >= 1980
and p.born <= 2000
return p.name, p.born
